import Foundation

// MARK: - Person
class Person {
    var name: String
    var friends: [Unowned<Person>] = []
    
    init(name: String) {
        self.name = name
        print("Hello: \(name)")
    }
    
    deinit {
        print("Goodbye \(name)")
    }
}

// MARK: - Unowned
final class Unowned<T: AnyObject> {
    unowned let value: T
    init(_ value: T) {
        self.value = value
    }
}

do {
    let ray = Person(name: "Ray")
    let vicki = Person(name: "Vicki")
    let greg = Person(name: "Greg")
    let tammy = Person(name: "Tammy")
    let darren = Person(name: "Darren")
    let richard = Person(name: "Richard")
    
    ray.friends.append(contentsOf: [vicki,tammy,richard,greg,darren].map{Unowned($0)})
    vicki.friends.append(contentsOf: [ray,tammy,richard,greg,darren].map{Unowned($0)})
}
print("Finish! Everyone must be deallocated")
